...
**Your AI writes code all day. opsroom shows you whether it's paying.**

A local-first operator console for solo founders and indie hackers. It fuses three things you already have — your **AI coding sessions** (Claude Code), your **git repos**, and your **markdown notes** (Obsidian or plain folders) — into one dashboard that answers the only question that matters:
...
**📊 ACTIVITY — where your time actually went.** Sessions per venture, commits, and the two features that hurt (in a good way):
...
| `chat` | optional manual drop of a chat export |
...
**Do I need Claude Code / Obsidian?** No. Every source is optional and degrades gracefully. Git repos alone give you drift + loops + sessions; add notes when you want the money features.

**Does it work with Cursor / other AI tools?** Today it parses Claude Code's JSONL logs. Collectors are ~80-line files with one `collect(con)` entry point — PRs welcome.
...
python tests/test_redaction.py && python tests/test_state.py && python tests/test_demo.py
